...
def process_powerpoint():
    ...
    try:
        ...
        flashcard_id = save_flashcards_txt(flashcards, file.filename, user_id=user['id'])
        if not flashcard_id:
...
def profile():
    ...
    flashcards = conn.execute('''
        SELECT f.flashcard_id, f.filename, f.created_at,
               (SELECT COUNT(*) FROM (
                   SELECT DISTINCT question, answer 
                   FROM json_each(
                       (SELECT json_group_array(json_object('question', question, 'answer', answer)) 
                        FROM (
                            SELECT substr(line, 1, instr(line, ':') - 1) as question,
                                   substr(line, instr(line, ':') + 1) as answer
                            FROM (
                                SELECT value as line
                                FROM json_each(readfile.read_lines)
                                WHERE value LIKE '%:%'
                            )
                        )
                       )
        ...
        FROM flashcards f
        CROSS JOIN (
            SELECT json_group_array(trim(line)) as read_lines
            FROM (
                SELECT line
                FROM (
                    SELECT readfile(ASSETS_DIR || '/flashcards_' || f2.flashcard_id || '.txt') as line
                    FROM flashcards f2
                    WHERE f2.flashcard_id = f.flashcard_id
                )
            )
        ) as readfile
        WHERE f.user_id = ?
    ...
    return render_template('profile.html', flashcards=flashcards, username=user['username'])
